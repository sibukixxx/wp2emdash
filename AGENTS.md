...
- `internal/domain/score/score.go`（および `score_test.go`）
...
global の `commit-conventions` skill を継承。本リポジトリでも同じ：
